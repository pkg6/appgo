...
func TestAssetBom(t *testing.T) {
	...
	if err != nil {
		...
		tc := &CarHeader{
			Tag:                bytess.NewString4("RATC"),
			CoreuiVersion:      691,
			StorageVersion:     17,
			StorageTimestamp:   0,
			RenditionCount:     7,
			MainVersionString:  bytess.NewString128("@(#)PROGRAM:CoreUI  PROJECT:CoreUI-691.2\n"),
			VersionString:      bytess.NewString256("Xcode 12.5 (12E262) via IBCocoaTouchImageCatalogTool"),
			UUID:               bytess.NewString16(""),
	...
	if c, err := b.ExtendedMetadata(); err != nil {
		...
		tc := &CarextendedMetadata{
			Tag:                       bytess.NewString4("META"),
			ThinningArguments:         bytess.NewString256(""),
			DeploymentPlatformVersion: bytess.NewString256("13.1"),
			DeploymentPlatform:        bytess.NewString256("ios"),
			AuthoringTool:             bytess.NewString256("@(#)PROGRAM:CoreThemeDefinition  PROJECT:CoreThemeDefinition-491\n"),
		}
		if !reflect.DeepEqual(c, tc) {
			t.Fail()
		}
